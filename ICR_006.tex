...
Summary statistics focus attention on a particular possible value of a distribution's parameter. In reality, a sample will suggest a range of plausible values, with each value having an associated probability. Additionally, we often know what values are realistic for a parameter - this knowledge is valuable and can be used to reduce the uncertainty of our estimates. In Bayesian inference, this probability distribution is known as the parameter's \emph{posterior}. It's the result of combining knowledge about the parameter available before data is collected, and the information in the data itself. Estimating the posterior, rather than the `most likely' parameter, has several benefits:
\begin{itemize}
	\item The uncertainty in a particular parameter estimate is immediately clear
	\item A posterior distribution provides the probability that a particular range of values contains the population parameter - this is much easier to interpret than an interval estimate.
\end{itemize}
Bayesian inference combines prior knowledge with test data to estimate a distribution's parameters. Knowing what parameters are plausible allows us to place bounds on the behaviour of a population.
