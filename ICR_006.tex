...
Summary statistics focus attention on a particular possible value of a distribution's parameter. In reality, a sample will suggest a range of plausible values, with each with an associated probability. In Bayesian inference, this probability distribution is known as the parameter's \emph{posterior}. It's the result of combining knowledge about the parameter available before data is collected, and the information in the data itself. Estimating the posterior, rather than the `most likely' parameter, has several benefits:
\begin{itemize}
	\item Its spread makes it clear how much uncertainty there is about what the true parameter value is.
	\item It tells us directly what the probability of the parameter being a particular value is, which is much easier to communicate to the uninitiated than a $p$-value, $t$-tests, and interval estimates.
	\item The assumptions made are arguably more evident than in hypothesis testing, and are consequently much easier to verify.
\end{itemize}


To retiterate, Bayesian inference combines prior knowledge with test data to estimate a distribution's parameters. Knowing what parameters are plausible allows us to place bounds on the behaviour of a population.

$p(\theta, y)$ is the probability that the success probability is $\theta$ and the data $y$ is observed. Bayes' rule relates this joint distribution to the probability of a value of $\theta$ given the data:
\[
  p(\theta|y) = \frac{p(y|\theta)\cdot p(\theta)}{\int_{\theta}p(y|\theta)\cdot p(\theta)\cdot d\theta}
\]
Qualitatively, this expression states that the probability of a parameter given the data (the \emph{posterior}) is directly proportional to the data's probability, given that parameter value (the \emph{likelihood}). The denominator of this expression is constant w.r.t. $\theta$, therefore we can reduce the above to 
\[
  p(\theta|y) \propto p(y | \theta)\cdot p(\theta)
\]

To estimate $p(\theta|y)$ then, we need two things:
* $p(y|\theta)$ - the probability of the data for a given probability of success (the \emph{likelihood}).
* $p(\theta)$ - the probability of a parameter value according to prior knowledge (the \emph{prior}).

The probability of $y$ successes in a fixed number of trials for a given probability of success $\theta$ is described by the binomial distribution
\[
  p(y|\theta) = \binom{n}{y} \theta^y (1 - \theta)^{n - y}
\]

The prior distribution, $p(\theta)$, is a way to encode our knowledge that of whether a unit will pass into the model. If all probabilities of success seem equally plausible, then it's reasonable to assume that $p(\theta)$ is uniform over $\theta \in [0, 1]$:
\[
  \theta \sim \text{Unif}(0, 1) : p(\theta) = 1, \theta \in [0, 1]
\]
...
It might be easier to understand this process as one of simulation.
