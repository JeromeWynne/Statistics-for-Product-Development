...
\end{itemize}
\vspace{-10pt}
 Regression analysis would allow DCA's engineers to answer questions like these. Knowing what parameters affect the product, and by how much,  focuses development on the things that matter, resuling in a better quality product that's less risky to develop.
 \par
 Regression analysis tries to estimate a response using a function of some predictor variables. Linear regression uses a linear function to predict the reponse. This may sound limiting, since in real life lots of relationships are nonlinear, but nonlinear relationships can be made linear by transformation.
 \par
  Say there's a quadratic relationship $y = x^2$: Plotting $y$ against $x$ makes the function appear parabolic. However, if instead $y$ is plotted against $x^2$, the function becomes linear. In other words, by applying a transformation to one of the basis vectors - squaring $x$ - we end up in a space where the variable's relationship is linear
 Linear models describe a response as a linear function of some parameters $\beta$, each weighted by possible predictors. An example of such a model would be:
\begin{equation}
