...
Typically a test can be viewed in terms of predictor and reponse variables: in a tensile test of a unit for instance, load might be viewed as the reponse and displacement as a predictor. Linear models describe a response as a linear function of some parameters $\beta$, each weighted by possible predictors:
\begin{equation}
	f(x) = \beta_0 + \beta_1 x_1 + \beta_2 e^{x_1^2}+ \beta_3 \text{log} x_2
\end{equation}
Note that the coefficients $\beta_i$ are linear, but the predictors $f(x_i)$ don't have to be (where $x_i$ is a measured quantity\footnote{This quantity can be discrete (e.g. Yes/no, $\{0, 1, 2, 3\}$) or continuous.}).
Fitting this model such that $f(x)$ tends to be close to $y$ adjusts the coefficients to reflect their associated predictors' influence on the response.
\newpage
