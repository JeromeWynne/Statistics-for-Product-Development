...
\vspace{-10pt}
 ...
 \par
 Regression estimates how a response changes with some inputs. Linear regression uses a linear function to predict the reponse: This may sound limiting, since in real life lots of relationships are nonlinear, but nonlinear relationships can be made linear by transformation.
\par
  Linear models describe a response $y$ as a linear function of some parameters $\beta_i$, each weighted by a corresponding input $x_i$. An example of such a model would be:
\begin{equation}
	\hat{y} = f(x_1, x_2) = \beta_0 + \beta_1 \cdot x_1 + \beta_2 \cdot e^{x_1}+ \beta_3 \cdot x_1 \cdot x_2
\end{equation}
Where $\hat{y}$ is the estimated response, $x_i$ is an input, and $\beta_j$ is the coefficient of the $j$th input. Note that while the coefficients $\beta_j$ are linear, the predictors can be a nonlinear function of the measurements.

This model can be fit by adjusting the $\beta_i$ values so that $\hat{y}$ is a good estimate of the true response. To do this, it's necessary to measure how inaccurate $\hat{y}$ is relative to $y_i$. One way is to use the residual sum of squares:
\begin{equation}
	\text{RSS}(\beta) = \sum_{i = 1}^m \Big(y_i - \sum_{j = 1}^p \beta_i x_{ij})^2
	\label{eq:rss}
\end{equation}
...
From a practical standpoint, regression models should be fit using software. Matlab, R, or Octave can all be used to fit regression models. By setting up the problem such that the $N$ observed responses are in a column vector $\mathbf{y}$, their associated $p$ inputs form the rows of a matrix $\mathbf{X}$, which is $N \times p$, and the $p$ coefficients are in a column vector $\beta$, it's possible to succinctly write the RSS criterion, then minimize it analytically:
\begin{align}
	\text{RSS}(\beta) &= (\mathbf{y - X}\beta)^T(\mathbf{y - X}\beta) \\
	\frac{\partial \text{RSS}}{\partial \beta} &= -2\mathbf{X}^T(\mathbf{y - X}\beta) = 0 \\
	\implies &\beta = (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T \mathbf{y} \label{eq:normal_eqtn}
\end{align}
...
As a case study to show why linear models are useful, consider a test in which the load delivered by three groups of ten units is measured. The difference between the groups is categorical: the groups correspond to three component variations $A, B,$ and $C$. Since there isn't a natural order to the variations, it's necessary to code the difference between the units in a sensible way. There are several ways to do this, although a simple indicator stating whether a unit has a particular modification is sufficient: this way, the regression coefficients will be the differerence between the average response overall and the average response within a group.
\begin{table}
\centering
...
\begin{gather}
	\hat{y} = \beta_0 x_A + \beta_{1}x_B + \beta_{2}x_C = x \beta \\[5mm]
	\mathbf{X} = 
		{\tiny
			\begin{bmatrix}
				x_1 \\ x_2 \\ \vdots \\ x_{30}
	...
	\quad \beta = {\tiny \begin{bmatrix} \beta_0 \\ \beta_1 \\ \beta_2 \end{bmatrix}}
\end{gather}
We can then use the normal equation (Equation \ref{eq:normal_eqtn}) to make the least-squares fit. As it happens in this case, $\beta_A$, $\beta_B$, and $\beta_C$ represent the average responses within each group. Something to be quite careful of is not to include redundancy in the 
...
% Why too many regression coefficients makes life hard
