...
Summary statistics focus attention on a particular possible value of a distribution's parameter. In reality, a sample will suggest a distribution for this parameter. In Bayesian inference, we seek to estimate this distribution, which is known as the parameter's \emph{posterior}. The posterior is the result of combining knowledge available before data is collected and the information in the data itself. Estimating the posterior, rather than the `most likely' parameter, has several benefits:
\begin{itemize}
	\item Its spread makes it clear how much uncertainty there is about what the true parameter value is.
	\item The distribution directly describes the probability of a particular parameter value, making it much easier to communicate to the uninitiated than hypothesis tests or interval estimates.
	\item The assumptions being made in Bayesian inference are clearer than in hypothesis testing.
\end{itemize}

\par
An example of where this would be applicable in DCA is in estimating the proportion of units that will pass a test.
In this instance, the data is the $y$ in $n$ units that pass in a sample, and the prior is what we think the probability of a given proportion $\theta$ passing is. All that's necessary to compute the posterior is to weight the data's probability by the prior probability:
\begin{equation}
  p(\theta|y) = \frac{p(y|\theta)\cdot p(\theta)}{\int_{\theta}p(y|\theta)\cdot p(\theta)\cdot d\theta}
  \label{eq:bayes}
\end{equation}
Expression (\ref{eq:bayes}) states that the probability of a parameter given the data (the \emph{posterior}) is directly proportional to the data's probability given that parameter value (the \emph{likelihood}). The denominator of this expression is constant w.r.t. $\theta$, so that
\begin{align}
...
\end{equation}
 An analytical solution can be difficult and tedious. In practice, the computational method is generally used. This consists of defining a grid of $\theta$ values, calculating the corresponding likelihoods and priors using a software package, then taking the product of these to get the posterior.
 \begin{enumerate}
 	\item Define a list  $\boldsymbol{\theta}$ of values that $\theta$ can take on, and variables representing the number of successes $y$ and the number of trials $n$. In this case, we could use $\boldsymbol{\theta} = \{0, 0.01, ..., 1\}$ since $\theta$ can take on values between 0 and 1.
 	\item Calculate the likelihood, $p(y|\theta)$, and the prior, $p(\theta)$, for each value in this list. Matlab and R have functions that return the probability distributions of common functions. Diligently then, we plug in our values to create two more lists, one for the likelihood probabilities, and one for the prior probabilities:
 	\begin{align}
 		\boldsymbol{\textbf{Pr}(y|\theta)} &= \texttt{pbinom}(y, n, \boldsymbol{\theta})  \nonumber \\
 		\boldsymbol{p(\theta)} &= \boldsymbol{1} \nonumber
 	\end{align}
 	\item Multiply the elements of these lists by each other, then divide by the sum of the result to get the posterior probabilities.
 	\begin{gather*}
 		\texttt{Unnormalized } \boldsymbol{p(\theta|y)} = \boldsymbol{p(y|\theta)} \odot \boldsymbol{p(\theta)} \\
 		\boldsymbol{p(\theta|y)} =  \frac{\texttt{Unnormalized } \boldsymbol{p(\theta|y)}}{\sum_{\theta} p(\theta|y)\cdot \delta \theta}
 	\end{gather*}
 \end{enumerate}
