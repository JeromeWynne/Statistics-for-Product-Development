...
Experiment design can be used to make products that perform better, are more reliable, less risky to develop, and have a uniquely justifiable development process. It is expertise that would elevate DCA's capacity as a technical consultancy.
\par
...
\par
Three principles guide the design of robust experiments:
\begin{description}
...
\subsection*{Summary Statistics}
 Randomness refers to variation in a response as a result of uncontrolled factors. If a coin is always flipped according to the same process, then it will always land in the same orientation. By contrast, if a coin is flipped by hand, then the person doing the flipping has a limited amount of control over the factors affecting the flip. As a result, there is uncertainty as to what the outcome of that flip will be. There are two ways to reduce this uncertainty: Either make assumptions about how those factors vary, or take measurements of them. The former method allows real-world knowledge to state what outcomes are possible what their relative propensities are. Conversely, measurements allow the factor effects to be estimated, meaning that sources of variation in the response can be accounted for.
 
  The assumption of normality, for example, is often wielded as a claim that... and the main factors are monitored or controlled in an experiment so that their effect on the response can be eliminated.
 By measuring all factors affecting the flip it would be possible to deduce how exactly how the coin would land
 
 In spite of its name, a random variable (r.v.) is a function that maps events onto real numbers. For example, we could define an r.v. $X$ that maps the outcomes of a coin toss onto the numbers 1 and 0:
 \begin{align}
...
\caption{Fifty tolerance limits versus population limit.}
...
\caption{99$\%$ confidence intervals for 100 10-unit samples.}
